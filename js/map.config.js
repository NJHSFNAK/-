...
function posCalcLnglat (prevElement, nextElement) {
  ...
  // x,y缩放比例
  xCalc = (Math.abs(pixel1.x - pixel0.x)) / (Math.abs(prevElement.pos[0] - nextElement.pos[0]))
  yCalc = (Math.abs(pixel1.y - pixel0.y)) / (Math.abs(prevElement.pos[1] - nextElement.pos[1]))
  // 地图上距离上、下、左、右的边界的距离
...
container.addEventListener('touchstart', function (e) {
  ...
  mapZoom = map.getZoom();
  centerPos = map.lngLatToPixel([Number(centerLngLat[0]), Number(centerLngLat[1])], mapZoom);
  mouseMove = true;
...
function moveFun (event) {
  ...
  if (mouseMove) {
    ...
    if (event.touches && event.touches.length == 1) {
      ...
      // 将移动的距离转为经纬度
      var moveLnglat = map.pixelToLngLat(new AMap.Pixel(mapLeft, mapTop), map.getZoom()).toString().split(','); // 返回 LngLat 对象
      // 平移经纬度
...
// 获取适合的级别和中心点经纬度
// var fitZoomAndCenter = map.getFitZoomAndCenterByBounds(map.getBounds(), [0, 0, 0, 0], 20)
// var initLnglat = fitZoomAndCenter[1].toString().split(',')
// map.setZoom(fitZoomAndCenter[0])
// map.setCenter([initLnglat[0], initLnglat[1]])
